...
mapping: dict[str, TuyaBLECategoryBinarySensorMapping] = {
    "common": TuyaBLECategoryBinarySensorMapping(
        mapping=[
            TuyaBLEBinarySensorMapping(
                dp_id=CONNECTED_DP_ID,
                description=BinarySensorEntityDescription(
                    key="connected",
...
def get_mapping_by_device(device: TuyaBLEDevice) -> list[TuyaBLEBinarySensorMapping]:
    ...
    if common_mapping := mapping.get("common"):
        if common_mapping.mapping:
            result.extend(common_mapping.mapping)
...
class TuyaBLEBinarySensor(TuyaBLEEntity, BinarySensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        elif self._mapping.dp_id == CONNECTED_DP_ID:
            self._attr_is_on = self._device.connected
        else:
